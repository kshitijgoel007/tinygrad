...
from tinygrad.helpers import DEBUG_MOVEMENT, prod, getenv, all_int, all_same, DEBUG, _METADATA, Metadata
...
class LazyBuffer:
  ...
  def _view(self, new_st:ShapeTracker, op="reshape") -> LazyBuffer:
    if DEBUG_MOVEMENT: print(f"{self.base.op}.{op} {self.st.shape} -> {new_st.shape}")
  ...
  def pad(self, arg:Tuple[Tuple[sint, sint], ...]): return self._view(self.st.pad(arg), "pad")
  def expand(self, arg:Tuple[sint, ...]): return self._view(self.st.expand(arg), "expand")
  def permute(self, arg:Tuple[int, ...]): return self._view(self.st.permute(arg), "permute")
  def shrink(self, arg:Tuple[Tuple[sint, sint], ...]): return self._view(self.st.shrink(arg), "shrink")
  def stride(self, arg:Tuple[int, ...]): return self._view(self.st.stride(arg), "stride")
